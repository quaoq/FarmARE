...
class ScenarioL2HBReplantCrustingReplantDecision(Scenario):
    ...
    def build_events_flow(self) -> None:
        ...
        if self.detailed_briefing:
            briefing_text = (
                "截至2026-05-22，黑农84全田播后仍有部分区域出苗偏慢，前期强雨后表土板结风险较高。"
                "请先复查天气、预报、土壤表层墒情、全田出苗概况和种子/设备资源；"
                "再用无人机和地面检查定位真正需要补种的区域，排除只是正常延迟出苗。"
                "如果确认局部stand缺口已经影响群体且剩余季节仍支持补种，请等待合适作业窗口后只补种确认区域，"
                "并复查补种后的出苗风险、资源余量和记录。"
            )
        else:
            briefing_text = "请诊断播后板结导致的局部出苗风险，确认区域和窗口后只对必要区域补种并复查。"
