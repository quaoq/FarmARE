...
SCENARIO_ID = "scenario_l2_hb_coldspring_heihe50_field_prep"


@register_scenario(SCENARIO_ID)
class ScenarioL2HBColdspringHeihe50FieldPrep(Scenario):
    """L2 split: cold-spring field prep, base fertilizer, and ridge forming."""
    ...
    def build_events_flow(self) -> None:
        ...
        if self.detailed_briefing:
            briefing_text = (
                "2026-05-05冷春黑河50田块尚未整地和播种。"
                "本L2只覆盖耕整地窗口：先复核当前天气、5日预报、种床土温/墒情、"
                "肥料/燃油库存和拖拉机状态；条件允许时完成平地、基肥和1.1 m起垄，"
                "作业后复查田块和设备状态。不要播种。"
            )
        else:
            briefing_text = "请完成冷春黑河50田块的整地、基肥和起垄，并复查；不要播种。"
        ...
        with EventRegisterer.capture_mode():
            ...
            o_report = aui.send_message_to_user(
                content="已完成冷春黑河50田块整地、基肥、起垄和复查；未播种。"
    ...
    def validate(self, env) -> ScenarioValidationResult:
        return validate_native_workflow(self, env, "HEIHE50 cold-spring field-prep L2")
