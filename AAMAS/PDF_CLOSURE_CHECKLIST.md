...
| Actual A2A delegation versus condition assignment | `matched_baselines.py` reports observed delegation independently of assigned mode | `test_confirmation_audit.py::test_a2a_assignment_is_not_evidence_that_delegation_occurred` | `baseline_compatibility_v3/results.jsonl`; `test_live_compatibility_repairs.py` | Correct public briefing verified in both v3 exports; A2A performed three completed expert calls in 12 requests, direct none. v3 still contains a structured-answer crash and default generation-setting mismatch; final repaired compatibility remains pending. No seasonal benefit is established. |
