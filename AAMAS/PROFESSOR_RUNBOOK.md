...
`HANDOVER_STATUS.md` before treating this as an executable handover. Commands
...
1. **Review and sign-off.** Review all three exact executable specifications,
   ...
   protocol. Follow `REVIEW_ROUTE.md`; author-defined artifacts are not independently
...
3. **Offline checks.** Run `uv run --frozen pytest are/simulation/tests/distributed -q`.
   Use the detailed doctor/preflight commands in `PROFESSOR_03_EXECUTION_GUIDE.md`
...
8. **Annotate.** Use `are-dcore validation --help` and `PAPER_FOUNDATION.md`: freeze
