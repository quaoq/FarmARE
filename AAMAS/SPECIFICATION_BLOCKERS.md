...
| --- | --- | --- |
| Wet-June | Richer hierarchical draft, native observations, receipts and omission checks | Audit each fact mapping, predicate and acceptance boundary against native mechanics; replace message-name fault targets with phase/route/send-order targets; declare sensitivity alternatives. A three-day delayed fixed workflow did not create a native rejection, so an execution-window/recovery specification cannot claim that mechanism is validated. |
| Disease–Drought | Native fixed workflow and synchronized daily hydrology records | Select a justified, separately versioned scenario that passes development screening, then define observable diagnostic prerequisites and their relationship to root-zone stress. The existing deep-root scenario is physically inactive at the target, while restricted-root candidates fail stress or complete paired harvest. The irrigation-benefit branch is not established. |
