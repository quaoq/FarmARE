...
def high_density_diagnostics(
    ...
    stress_actions = {
        ...
        in {"apply_fungicide", "apply_pesticide", "spray_pesticide"}
    }
    if stress_actions:
        warnings.append(
            f"high-density baseline contains biotic stress actions: {sorted(stress_actions)}"
        )
    ...
    if max_ndvi > 0.96:
        warnings.append(
            f"high-density baseline NDVI unexpectedly saturated: {max_ndvi:.3f}"
        )
    return warnings
