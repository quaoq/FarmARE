...
from are.simulation.scenarios.scenario_farm_world_fullseason_v2.scenario_full_season_heinong84_edge_low_fertility import (  # noqa: E402
    EDGE_END,
    EDGE_START,
    SCENARIO_ID,
    SEVERE_EDGE_END,
    SEVERE_EDGE_START,
    ScenarioFullSeasonHeinong84EdgeLowFertility,
)
...
class Heinong84EdgeLowFertilityDailyTraceApp(App):
    ...
    def capture_daily_state(
        ...
        zone_acc = {
            "edge_0_11": _new_accumulator(),
            "severe_0_3": _new_accumulator(),
            "mild_edge_4_11": _new_accumulator(),
            "healthy_12_63": _new_accumulator(),
        }
...
class TraceScenario(ScenarioFullSeasonHeinong84EdgeLowFertility):
    def init_and_populate_apps(self, *args: Any, **kwargs: Any) -> None:
        super().init_and_populate_apps(*args, **kwargs)
        farm_world = self.get_typed_app(FarmWorldApp)
        weather = self.get_typed_app(WeatherApp)
        self._trace_app = Heinong84EdgeLowFertilityDailyTraceApp(
...
def _summarize(acc: dict[str, Any]) -> dict[str, Any]:
    return {
        ...
        "avg_stand_fraction": round(_mean(acc["stand_fraction"]), 4),
        "min_stand_fraction": round(_min(acc["stand_fraction"]), 4),
...
def _primary_zone_name(ridge_id: int) -> str:
    if SEVERE_EDGE_START <= ridge_id <= SEVERE_EDGE_END:
        return "severe_0_3"
    if EDGE_START <= ridge_id <= EDGE_END:
        return "mild_edge_4_11"
    return "healthy_12_63"
...
def _zones_for_ridge(ridge_id: int) -> list[str]:
    if EDGE_START <= ridge_id <= EDGE_END:
        zones = ["edge_0_11"]
        if SEVERE_EDGE_START <= ridge_id <= SEVERE_EDGE_END:
            zones.append("severe_0_3")
        else:
            zones.append("mild_edge_4_11")
        return zones
    return ["healthy_12_63"]
...
def _yield_summary(scenario: Any) -> dict[str, Any]:
    ...
    zone_totals = {
        "edge_0_11": {"biological_kg": 0.0, "recovered_kg": 0.0},
        "severe_0_3": {"biological_kg": 0.0, "recovered_kg": 0.0},
        "healthy_12_63": {"biological_kg": 0.0, "recovered_kg": 0.0},
    }
    bio_total = 0.0
    rec_total = 0.0
    planted = 0
    ...
    for rid, yld in physics.yield_recovery.states.items():
        ...
        if yld.harvested:
            harvested += 1
        if EDGE_START <= rid <= EDGE_END:
            zone_totals["edge_0_11"]["biological_kg"] += bio
            zone_totals["edge_0_11"]["recovered_kg"] += rec
            if SEVERE_EDGE_START <= rid <= SEVERE_EDGE_END:
                zone_totals["severe_0_3"]["biological_kg"] += bio
                zone_totals["severe_0_3"]["recovered_kg"] += rec
        else:
            zone_totals["healthy_12_63"]["biological_kg"] += bio
            zone_totals["healthy_12_63"]["recovered_kg"] += rec
...
def main() -> int:
    ...
    parser.add_argument(
        "--field-csv",
        type=Path,
        default=Path("docs/ai/heinong84-edge-low-fertility-field-summary.csv"),
    )
    parser.add_argument(
        "--ridge-csv",
        type=Path,
        default=Path("docs/ai/heinong84-edge-low-fertility-ridge-states.csv"),
    )
    parser.add_argument(
        "--trace-json",
        type=Path,
        default=Path("docs/ai/heinong84-edge-low-fertility-oracle-trace.json"),
    )
